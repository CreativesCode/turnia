...
export default function StatisticsGeneralPage() {
  const { orgId, isLoading: orgLoading, error: orgError, canManageOrg } = useScheduleOrg();
  ...
  // Estadísticas generales de toda la organización (sin filtrar por usuario)
  const { data, isLoading, error, refetch } = useStatistics(orgId, dateRange.start, dateRange.end, undefined);
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-xl border border-border bg-background p-6">
          ...
          <p className="mt-2 text-sm text-muted">
            Solo los administradores pueden ver las estadísticas generales de la organización.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <StatisticsFilters startDate={dateRange.start} endDate={dateRange.end} onDateRangeChange={handleDateRangeChange} showUserFilter={false} />
