...
function AdminShiftTypesContent() {
  ...
  if (!orgId && !isSuperadmin) {
    return (
      <div>
        <p className="text-text-secondary">No tienes permisos para gestionar tipos de turno.</p>
        <Link href="/dashboard/admin" className="mt-2 inline-block text-primary-600 hover:text-primary-700">
          ← Admin
        </Link>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="md:hidden">
        ...
        <h1 className="mt-2 text-xl font-semibold text-text-primary">Tipos de turno</h1>
        <p className="mt-1 text-sm text-text-secondary">
          Define los tipos (Mañana, Noche, 24h, etc.) con letra y color. Son necesarios para crear turnos.
        </p>
      </div>
      ...
      <ShiftTypesList orgId={effectiveOrgId} refreshKey={listRefreshKey} onRefresh={onRefresh} />
    </div>
  );
}
