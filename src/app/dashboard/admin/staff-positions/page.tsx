...
import { StaffPositionsList } from '@/components/staff-positions/StaffPositionsList';
...
function AdminStaffPositionsContent() {
  ...
  if (!orgId && !isSuperadmin) {
    return (
      <div>
        <p className="text-text-secondary">No tienes permiso para gestionar puestos de personal.</p>
        <Link href="/dashboard/admin" className="mt-2 inline-block text-primary-600 hover:text-primary-700">
          ← Admin
        </Link>
      </div>
  ...
  return (
    <div className="space-y-6">
      <DashboardDesktopHeader
        title="Puestos de personal"
        subtitle="Define puestos como Médico Turnate, Médico de refuerzo, etc."
      />
      ...
      <div className="md:hidden">
        ...
        <h1 className="mt-2 text-xl font-semibold text-text-primary">Puestos de personal</h1>
        <p className="mt-1 text-sm text-text-secondary">
          Define los puestos para asignar a los miembros (ej. Médico Turnate, Médico de refuerzo).
        </p>
      </div>
      ...
      <StaffPositionsList orgId={effectiveOrgId} refreshKey={listRefreshKey} onRefresh={onRefresh} />
    </div>
  );
}

export default function AdminStaffPositionsPage() {
  return (
    <Suspense
      fallback={
        <div className="space-y-6">
