...
function AdminExportsContent() {
  ...
  if (!orgId && !isSuperadmin) {
    return (
      <div>
        <p className="text-text-secondary">No tienes permisos para exportar horarios.</p>
        <Link href="/dashboard/admin" className="mt-2 inline-block text-primary-600 hover:text-primary-700">
          ← Admin
        </Link>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="md:hidden">
        ...
        <h1 className="mt-2 text-xl font-semibold text-text-primary">Exportar horarios</h1>
        <p className="mt-1 text-sm text-text-secondary">
          Descarga los turnos del período en CSV o Excel (fecha, horario, tipo, asignado, ubicación).
        </p>
      </div>
      ...
      <ExportScheduleForm orgId={effectiveOrgId} />
    </div>
  );
}
